...
function formatOneAlignment(oneAlignment){
	...
	for(var i =0; i < oneAlignment.length; i++){
		...
		sequence0+=("<td "+"class='oneAlignmentTd'>"+"<p style='margin-top:10px; margin-bottom:0px; padding:0px; "+pairColors[0]+"'>"+oneAlignment[i][0]+"</p>"+"<p style='margin:0px; padding:0px; "+pairColors[1]+"'>"+oneAlignment[i][1]+"</p>"+"</td>");
	}
	sequence0 += "</tr>";
	sequence1 += "</tr>";
...
function render(AllAlignments, sequenceNames, userFile){
	...
	var simulation = d3.forceSimulation()
			.force("link", d3.forceLink().id(function(d) { return d.id; }).distance(width / 6).strength(2))
	  ...
	  var localGraph = {
		...
		var text = svg.append("g")
		     ...
		     .data(localGraph.nodes)
		  	 .enter().append("text")
	       .attr("dx", 6)
				 .attr("fill", "black").style("text-anchor", "start")
         .attr("y", -20)
	       .text(function(d) {return d.name; });

      simulation.nodes(localGraph.nodes)
			     .on("tick", ticked)
